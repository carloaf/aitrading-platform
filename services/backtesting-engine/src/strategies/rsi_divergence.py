...
@dataclass
class PeakValley:
    """Representa um pico ou vale identificado"""
    index: int
    price: float
    rsi_value: float
    timestamp: str
    type: str  # 'peak' ou 'valley'
...
class RSIDivergenceStrategy(BaseStrategy):
    ...
    def __init__(self, parameters: Dict[str, Any] = None):
        ...
        # Cache de picos e vales
        self._price_peaks: List[PeakValley] = []
        self._price_valleys: List[PeakValley] = []
        self._rsi_peaks: List[PeakValley] = []
        self._rsi_valleys: List[PeakValley] = []
    ...
    def _find_divergence(self, df: pd.DataFrame, idx: int) -> Optional[DivergencePattern]:
        ...
        lookback = self.parameters['lookback_periods']
        min_distance = self.parameters['min_peak_distance']
        ...
        # Preço faz mínimas mais altas, RSI faz mínimas mais baixas (continuação de alta)
        if len(price_valleys) >= 2 and len(rsi_valleys) >= 2:
            ...
            trend_bullish = current_price > df['SMA_trend'].iloc[idx]
        ...
        # Preço faz máximas mais baixas, RSI faz máximas mais altas (continuação de baixa)
        if len(price_peaks) >= 2 and len(rsi_peaks) >= 2:
            ...
            # Confirmar tendência de baixa (preço abaixo da MA)
            trend_bearish = current_price < df['SMA_trend'].iloc[idx]
    ...
    def _calculate_divergence_strength(
        ...
        if div_type in ['bullish', 'hidden_bullish']:
            # Para sinais de compra, RSI baixo é melhor
            rsi_zone_score = max(0, (self.parameters['rsi_oversold'] - current_rsi + 20) / 40)
        else:
            # Para sinais de venda, RSI alto é melhor
        ...
        if 'MACD_hist' in df.columns:
            ...
            if div_type in ['bullish', 'hidden_bullish']:
                # Para compra, MACD crescendo é bom
                macd_score = 1.0 if macd_hist > macd_hist_prev else 0.3
            else:
                # Para venda, MACD caindo é bom
                macd_score = 1.0 if macd_hist < macd_hist_prev else 0.3
        else:
            macd_score = 0.5
    ...
    def _apply_filters(self, df: pd.DataFrame, idx: int, divergence: DivergencePattern) -> bool:
        ...
        if df['ADX'].iloc[idx] < self.parameters['min_adx_trend']:
            logger.debug(f"Sinal filtrado: ADX muito baixo ({df['ADX'].iloc[idx]:.2f})")
            return False
        
        # 2. Filtro de volume (se habilitado)
        if self.parameters['volume_confirmation']:
            if df['Volume_ratio'].iloc[idx] < self.parameters['volume_multiplier']:
                # Permitir sinais com volume normal para divergências fortes
                if divergence.strength < 0.7:
                    logger.debug(f"Sinal filtrado: Volume insuficiente")
                    return False
    ...
    def get_pattern_statistics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for pattern_type in signals_df['signal_type'].unique():
            ...
            stats['pattern_distribution'][pattern_type] = {
                ...
                'avg_strength': signals_df[signals_df['signal_type'] == pattern_type]['signal_strength'].mean()
            }
