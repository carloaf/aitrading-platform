...
class MeanReversionStrategy(BaseStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Condições de COMPRA (preço na banda inferior)
        buy_condition = (
            (df['Close'] <= df['BB_lower'] * 1.01) &  # Toca banda inferior (+1% tolerância)
            (df['RSI'] < rsi_oversold) &  # RSI confirma sobrevenda
            (df['BB_position'] < 0.2) &  # Preço nos 20% inferiores das bandas
            (df['Stoch'] < 20)  # Stochastic também sobrevenda
        )
