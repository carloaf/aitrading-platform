...
class MonteCarloSimulator:
        ...
        try:
            ...
            for i, row in signals.iterrows():
                ...
                if signal == 'BUY' and position == 0:
                    # Enter long position
                    position = balance / price * 0.99  # 1% slippage/fee
                    entry_price = price
                    balance = 0
                    
                elif signal == 'SELL' and position > 0:
                    # Exit position
                    balance = position * price * 0.99  # 1% slippage/fee
                    pnl = balance - self.initial_balance
                    pnl_pct = (balance / self.initial_balance - 1) * 100
            ...
            if position > 0:
                balance = position * signals.iloc[-1]['close'] * 0.99
                pnl = balance - self.initial_balance
                pnl_pct = (balance / self.initial_balance - 1) * 100
                trades.append({
                    'entry': entry_price,
                    'exit': signals.iloc[-1]['close'],
                    'pnl': pnl,
                    'pnl_pct': pnl_pct
                })
    ...
    def run_simulation(self,
                      ...
                      strategy_func: callable,
                      param_ranges: Dict[str, Tuple[float, float]],
                      parallel: bool = True) -> MonteCarloReport:
        """
